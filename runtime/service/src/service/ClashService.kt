                ...
                launch {
                    runCatching {
                            ...
                            startupLogStore.append(
                                "LOCAL_HTTP spec: create done profile=${spec.profileUuid} overrides=${spec.overrideSpecs.size}"
                            )
    ...
    private fun scheduleReload() {
        ...
        reloadJob = launch {
            ...
            val spec =
                runCatching { SessionRuntimeSpecFactory(appContextOrSelf).createHttpSpec() }
                    .getOrElse { error ->
                        ...
                        startupLogStore.append(
                            "LOCAL_HTTP failed=${error.message ?: "http runtime spec refresh failed"}"
                        )
                        Log.w("HTTP runtime spec refresh failed: ${error.message}")
                        return@launch
                    }
            startupLogStore.append(
                "LOCAL_HTTP spec: reload create done profile=${spec.profileUuid} overrides=${spec.overrideSpecs.size}"
            )
            ...
            if (!result.success) {
                reason = result.error
                startupLogStore.append(
                    "LOCAL_HTTP failed=${result.error ?: "http runtime reload failed"}"
                )
